...
control <- trainControl(
  ...
  search = "random",             # zufällige Suche
  savePredictions = "final",
  allowParallel = TRUE
)

# -------------------------------------------------------
# Parameterbereich (schlank, aber gezielt)
# -------------------------------------------------------
...
cat(
  ...
  "Cores: ", n_cores, "\n",
  "CV-Folds: ", control$number, "\n",
  "Max Random Combos: 50\n",
...
# -------------------------------------------------------
# Training mit Random Search (max 50 Zufalls-Kombos)
# -------------------------------------------------------
model_xgb_tuned_ <- train(
  y ~ ., data = train_data,
  method = "xgbTree",
  trControl = control,
  tuneGrid = xgb_grid,
  metric = "ROC",
  tuneLength = 50,                        # <— nur 50 zufällige Kombis
...
# -------------------------------------------------------
best <- model_xgb_tuned_$bestTune
oof <- subset(model_xgb_tuned_$pred,
...
# -------------------------------------------------------
pred_train_prob  <- predict(model_xgb_tuned_, train_data, type="prob")[, "pos"]
pred_train_class <- factor(ifelse(pred_train_prob >= best_thr, "pos","neg"), levels=c("neg","pos"))

pred_test_prob   <- predict(model_xgb_tuned_, test_data,  type="prob")[, "pos"]
...
# Modell speichern
saveRDS(model_xgb_tuned_, file.path(out_dir, "model_xgb_tuned_.rds"))
