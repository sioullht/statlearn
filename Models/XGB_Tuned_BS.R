...
# Pakete
# -------------------------------------------------------
packages <- c("tidyverse","pROC","xgboost","ggplot2","caret")
...
# ROC / AUC
roc_train <- pROC::roc(y_train, pred_train_prob, quiet = TRUE)
roc_test  <- pROC::roc(y_test,  pred_test_prob,  quiet = TRUE)
...
# -------------------------------------------------------
# Bootstrapping (B=100 Wiederholungen auf TEST-Set)
# -------------------------------------------------------
B <- 100
set.seed(123)
boot_results <- replicate(B, {
  idx <- sample(seq_along(y_test), replace = TRUE)
  y_b <- y_test[idx]; p_b <- pred_test_prob[idx]
  
  roc_b <- tryCatch(pROC::roc(y_b, p_b, quiet = TRUE), error=function(e) NA)
  auc_b <- if (!is.na(roc_b)[1]) as.numeric(pROC::auc(roc_b)) else NA
  acc_b <- mean((p_b >= best_thr) == y_b)
  logloss_b <- logloss(y_b, p_b)
  brier_b   <- brier(y_b, p_b)
  
  c(Accuracy=acc_b, AUC=auc_b, Log_Loss=logloss_b, Brier_Score=brier_b)
})
...
boot_summary <- boot_df %>%
  summarise(across(
    ...
    list(
      ...
      CI_U  = ~ quantile(.x, 0.975, na.rm = TRUE)
    )
  ))

readr::write_csv(boot_df, file.path(out_dir, "bootstrap_raw.csv"))
readr::write_csv(boot_summary, file.path(out_dir, "bootstrap_summary.csv"))
